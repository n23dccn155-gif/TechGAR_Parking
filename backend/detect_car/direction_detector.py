...
class DirectionDetector:
    ...
    def _check_line_crossing(self, prev_pos: Tuple[int, int], curr_pos: Tuple[int, int]) -> Optional[str]:
        """
        Kiểm tra xe có cắt qua ROI line nào giữa prev_pos → curr_pos.
        Trả về roi_id nếu cắt, None nếu không.
        """
    ...
    def update(self, tracks: dict, frame_idx: int) -> List[dict]:
        ...
        for tid, track in tracks.items():
            ...
            roi_id = self._check_line_crossing(prev_pos, curr_pos)
            if roi_id is None:
                continue
        ...
        for pd in self._pending:
            ...
            if pd.track_id in tracks:
                ...
                # Đã đủ frame để quyết định?
                if len(pd.positions_after) >= self.decision_frames:
                ...
                # Track đã bị xóa → quyết định UNKNOWN
                pd.decided = True
                pd.decision = "UNKNOWN"
