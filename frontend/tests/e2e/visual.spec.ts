...
test("capture required mobile and desktop screenshots", async ({ page }, testInfo) => {
  ...
  await page.screenshot({ path: testInfo.outputPath("390x844-entry.png"), fullPage: false });
  ...
  await page.screenshot({ path: testInfo.outputPath("390x844-recommendation.png"), fullPage: false });

  await page.getByTestId("recommendation-confirm").click();
  await expect(page.getByTestId("active-route")).toBeVisible();
  await page.screenshot({ path: testInfo.outputPath("390x844-navigation.png"), fullPage: false });
  ...
  await page.screenshot({ path: testInfo.outputPath("1440x900-desktop.png"), fullPage: false });

  await page.getByTestId("spot-C06").click();
  await page.getByTestId("spot-navigate").click();
  await expect(page.getByTestId("active-route")).toBeVisible();
  await page.screenshot({ path: testInfo.outputPath("1440x900-c06-navigation.png"), fullPage: false });
});
